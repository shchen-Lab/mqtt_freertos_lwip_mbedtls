...
const int32_t test_ca_pem_len = sizeof(test_cli_ca);
...
int NetworkConnect(Network* n, char* addr, int port)
{
	...
	if (n->rootCA != NULL) {
		...
		if (mbedtls_x509_crt_parse( root_crt, (const unsigned char *)test_cli_ca, test_ca_pem_len ) != 0) {
			mqtt_printf(MQTT_DEBUG, "parse root_crt failed!");
			goto err;
		}
